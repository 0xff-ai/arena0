...
mod tests {
    ...
    #[test]
    fn callout_spec_decodes_exact_output_type() {
        let request = callouts::ChooseMove {
            round: 1,
            total_rounds: 3,
            your_score: 0,
            their_score: 0,
        ...
        let callout =
            <callouts::ChooseMove as CalloutSpec<RockPaperScissors>>::into_callout(request);
        assert!(matches!(callout, Callout::ChooseMove { .. }));

        let input =
            <callouts::ChooseMove as CalloutSpec<RockPaperScissors>>::into_input(Choice::Rock);
        let choice =
            <callouts::ChooseMove as CalloutSpec<RockPaperScissors>>::decode(input).unwrap();
        assert!(matches!(choice, Choice::Rock));
    }

    #[test]
    fn typed_callout_future_uses_exact_output_type() {
        let mut ctx = Context::<Shared>::__new(Shared::default(), (), peer_a());
        let future: ArenaFuture<Choice> = ctx
            .effects()
            .callout(callouts::ChooseMove {
                round: 1,
                total_rounds: 3,
                your_score: 0,
                their_score: 0,
        ...
        match future.into_effect() {
            arena0::types::Effect::Callout {
                callout_index,
                pending_label,
                expected_type,
                ..
                ...
                assert_eq!(
                    callout_index,
                    <callouts::ChooseMove as CalloutSpec<RockPaperScissors>>::CALLOUT_INDEX
                );
                assert_eq!(pending_label.as_deref(), Some("choosing-move"));
                assert_eq!(
