...
stop that item. Write `impl/conflict-d2b.md` with the item, the exact obstacle
(file:line and the error) and the options you see, then end your turn. Items
...
- Attribute paths are string `ExprPath`s with a turbofish:
  ...
  Import the module (`use arena0_program::bounded;` or `use crate::bounded;`)
...
`cargo fmt --all`, `just build-programs` (the SDK and ABI changed),
`just check`, `cargo test -p arena0-program -p arena0-protocol -p arena0-sandbox -p arena0-store`,
`just test`. Write `impl/report-d2b.md` with:
